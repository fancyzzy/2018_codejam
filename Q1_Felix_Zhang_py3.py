...
def leastInterval(tasks, n):
	...
	task_list = tasks[:]
	#for the n interval window that contains different task elements
	interval_list = []
	#for the tasks final sequence
	final_list = []

	#The most number task element
	most_num_task = None
	...
	while 1:
		...
		if len(task_list)!= 0:
			most_num_task = max(task_list, key=lambda x: x not in interval_list\
		 	and task_list.count(x))
			if most_num_task not in interval_list:
				final_list.append(most_num_task)
				interval_list.append(most_num_task)
				task_list.remove(most_num_task)

				if len(task_list) == 0:
					break
				#fill the interval
				#print("DEBUG start fill the {} intervals".format(n))
				for i in range(n):
					most_num_task = max(task_list, key=lambda x:\
					 x not in interval_list    and task_list.count(x))
					if most_num_task not in interval_list:
						final_list.append(most_num_task)
						interval_list.append(most_num_task)
						task_list.remove(most_num_task)
					...
					#print("DEBUG fill interval final_list: {}".\
						#format(final_list))
				#release the interval
				interval_list = []

			else:
				#There is no available task element
				#Just fill idle _
				final_list.append('_')
				interval_list.remove(interval_list[0])
		else:
	...
	#print("DEBUG: final_list: {}".format(final_list))
...
if __name__ == '__main__':
	...
	n = 1
	print("DEBUG task len = ", len(tasks))
